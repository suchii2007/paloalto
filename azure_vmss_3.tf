...
# Refer Existing Public Load Balancer
...
# Refer Existing Private Load Balancer
data "azurerm_lb" "private" {
  name                = var.private_lb_name
  resource_group_name = var.resource_group_name
}

resource "azurerm_lb_backend_address_pool" "pvtbpepool" {
  loadbalancer_id = data.azurerm_lb.private.id
  name            = "fw-pvt-backend-pool"
}

resource "azurerm_lb_probe" "pvt" {
  loadbalancer_id     = data.azurerm_lb.private.id
  name                = "fw-pvt-health-probe"
  port                = var.pvt_port
}

resource "azurerm_lb_rule" "lbpvtrule" {
  loadbalancer_id                = data.azurerm_lb.private.id
  name                           = "fw-pvt-lb-rule"
  protocol                       = "All"
  backend_address_pool_ids       = [azurerm_lb_backend_address_pool.pvtbpepool.id]
  frontend_ip_configuration_name = data.azurerm_lb.private.frontend_ip_configuration[0].name
  probe_id                       = azurerm_lb_probe.pvt.id
  frontend_port                  = "0"
  backend_port                   = "0"
}
...
resource "azurerm_virtual_machine_scale_set" "vmss" {
  ...
  network_profile {
    ...
    ip_configuration {
      name                                   = "IPConfiguration"
      subnet_id                              = var.subnet_ids[0]
      load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.pvtbpepool.id]
      primary                                = true
    }
  }
}
